# BloxBot autonomous asset pipeline — Work handoff
...
At handoff, branch `feature/v1-job-core` is 32 commits ahead of `main` and 0 behind.
...
If actual Blender or Roblox Studio execution cannot be performed in the Work environment, still complete the code path, automated unit/integration tests with fakes at adapter boundaries, and document the exact local smoke-test steps. Never fabricate a successful real-world run.
